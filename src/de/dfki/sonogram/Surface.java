...
public class Surface extends JFrame {
	...
	if (reftomain.gad.cpercoord.isSelected()==true) {
        ...
        SurfacePane sp = new SurfacePane(reftomain,this,false);
        if (sp.getGeometry() == null) {
            return (null);
        }
        objTransform.addChild(sp);
        SurfacePane spFlat = new SurfacePane(reftomain,this,true);
        if (sp.getGeometry() == null) {
            return (null);
        }
   ...
   //--------------------------------------------------------------------------------------------
	...
	public void run() {
		...
		if (time > 200 && reftomain.gad.cscrennsaver.isSelected()) {
		    Transform3D tr = new Transform3D();
		    tr.rotZ(Math.toRadians(0.15));
   ...
   //--------------------------------------------------------------------------------------------
   // a simple scene and attach it to the virtual universe
    public Surface(Sonogram sono) {
        reftomain = sono;
        System.out.println("--> Painting Surface");
        BranchGroup scene = createSceneGraph();
        if (scene == null) {
            dispose();   // remove this class from memory
            return;
        }
        setSize (400,400);
        setLocation(150,150);
	...
	// 	canvas3D.getView().setSceneAntialiasingEnable(reftomain.iperantialias==true); 
