...
public class SurfacePane extends Shape3D {
    ...
    public SurfacePane(Sonogram sono, Surface sf, boolean flat) {
	renderFlat = flat;
        reftomain = sono;
        reftosurface = sf;
        generateColors();
    ...
    private Geometry surfacePaneGeometry() {
        ...
        QuadArray qar;
        float[]   tempSpektrum;
        float[]   tempSpektrumlower;
        int       count   = 0;
        int       lx      = reftomain.spektrum.size();
        int       ly      = reftomain.timewindowlength/2;
        ...
        if (quads/reftomain.gad.slidersurface.getValue() > 30000 && renderFlat == false) {
            System .out.println("--> Large Plott");
            if ((JOptionPane.showConfirmDialog
                    (reftomain,"This Perspectogram is very large and your Graphic Card could\nhave problems to render this large Polygon Model in a proper time!\nThe Model has about "+quads/reftomain.gad.slidersurface.getValue()+" Polygons. Please reduce the Overlapping\nor Zoom In to reduce the number of Polygons for this Perspectogram.\nFor Point Clouds you could use Perspectogram Point density reduction.\nWould you like to continue calculating this Perspectogram ?","Generate very Large Perspectogram ?",0,3))==1) {
